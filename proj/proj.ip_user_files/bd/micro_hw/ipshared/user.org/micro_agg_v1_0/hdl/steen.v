...
module steen(clk, rst_n, in, valid_in, last_in, out, valid_out, last_out);
...
*/
	reg [`DATA_W-1:0] fifo [`S_DEPTH_W-1:0];
	reg [`S_H_PTR-1:0] head;
	reg [`S_T_PTR-1:0] tail0, tail1;
...
always @(posedge clk) begin
	if (!rst_n) begin
		...
		if (S0_enable) begin
			...
			fifo[head] <= in;
			out <= (fifo[tail1+8] > fifo[tail0]) ? fifo[tail1+8] : fifo[tail0];
			valid_out <= 1;
			last_out <= 0;
			head <= (we) ? head + 1 : head;
			tail0 <= (fifo[tail1+8] > fifo[tail0]) ? tail0 : tail0 + 1;
			tail1 <= (fifo[tail1+8] > fifo[tail0]) ? tail1 + 1 : tail1;
		end
...
assign S0toS1 = (state == S0) & (head == 7);
assign S1toS2 = (state == S1) & !(in > fifo[tail0]);
assign S1toS3 = (state == S1) & (in > fifo[tail0]) & (tail1 == 7);
assign S2toS3 = (state == S2) & (fifo[tail1+8] > fifo[tail0]) & (tail1 == 7);
assign S2toS4 = (state == S2) & !(fifo[tail1+8] > fifo[tail0]) & (tail0 == 7);
assign S3toS1 = (state == S3) & (tail0 == 7) & !delay_last;
assign S3toS0 = (state == S3) & (tail0 == 7) & delay_last;
assign S4toS1 = (state == S4) & (tail1 == 7) & !delay_last;
assign S4toS0 = (state == S4) & (tail1 == 7) & delay_last;
