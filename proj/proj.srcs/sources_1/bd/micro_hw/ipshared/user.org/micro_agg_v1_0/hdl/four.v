...
module four(clk, rst_n, in, valid_in, last_in, out, valid_out, last_out);
...
*/
	reg [`DATA_W-1:0] fifo [`F_DEPTH_W-1:0];
	reg [`F_H_PTR-1:0] head;
	reg [`F_T_PTR-1:0] tail0, tail1;
...
*/
	...
	wire [`DATA_W-1:0] mem0, mem1, mem2, mem3;
...
always @(posedge clk) begin
	if (!rst_n) begin
		...
		if (S0_enable) begin
			...
			fifo[head] <= in;
			out <= (fifo[tail1+2] > fifo[tail0]) ? fifo[tail1+2] : fifo[tail0];
			valid_out <= 1;
			last_out <= 0;
			head <= (we) ? head + 1 : head;
			tail0 <= (fifo[tail1+2] > fifo[tail0]) ? tail0 : tail0 + 1;
			tail1 <= (fifo[tail1+2] > fifo[tail0]) ? tail1 + 1 : tail1;
		end
...
assign S0toS1 = (state == S0) & (head == 1);
assign S1toS2 = (state == S1) & !(in > fifo[tail0]);
assign S1toS3 = (state == S1) & (in > fifo[tail0]) & (tail1 == 1);
assign S2toS3 = (state == S2) & (fifo[tail1+2] > fifo[tail0]) & (tail1 == 1);
assign S2toS4 = (state == S2) & !(fifo[tail1+2] > fifo[tail0]) & (tail0 == 1);
assign S3toS1 = (state == S3) & (tail0 == 1) & !delay_last;
assign S3toS0 = (state == S3) & (tail0 == 1) & delay_last;
assign S4toS1 = (state == S4) & (tail1 == 1) & !delay_last;
assign S4toS0 = (state == S4) & (tail1 == 1) & delay_last;
